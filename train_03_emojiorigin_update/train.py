...
from train_03_emojiorigin_update.lstm_emoji_attention import EMOJI_ATTENTION_LSTM
from train_03_emojiorigin_update.word_and_emoji_embedding import Tensor
...
# 真正用模型训练的地方
def train(model, data, optimizer, criterion,device,hidden):
    print("开始训练")
    epoch_loss = 0
    epoch_acc = 0
    model.train()
...
def run_with_valid_iterator(model, model_path, optimizer, criterion, train_data, valid_data, N_EPOCHS,device,lossType):
    ...
    for epoch in range(N_EPOCHS):
        ...
        train_loss, train_acc, optimizer, model, criterion = train(model, train_data, optimizer, criterion,device,hidden)
