...
class Tensor:
    ...
    valid_data = ''
    batch_size = ''
    path = os.getcwd()[0:os.getcwd().rfind("/")] + '/data/nlpcc2014/data_hasSplit/'
    ...
    def batch_iterator(self,data,batch_size):
        return BucketIterator(data,batch_size=batch_size,device= torch.device("cuda:0" if torch.cuda.is_available() else"cpu"))

    def train_iterator(self,train_data=None):
        if train_data is None:
            train_data = self.train_data
        # 4 batching 操作：用 torchtext 提供的 API 来创建一个 iterator
        train_iterator = BucketIterator(train_data,
                                batch_size=self.batch_size,
                                        device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) # cpu by -1, gpu by 0
                                # sort_key=lambda x: len(x.source), # field sorted by len
                                # sort_within_batch=True,
                                # repeat=False)
        return train_iterator

    def test_iterator(self,test_data):
    ...
    def get_emoji_vocab(self):
        vocab = self.EMOJI.vocab
        return vocab
