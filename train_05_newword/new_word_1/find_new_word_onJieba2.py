...
不设置阈值，结巴分词后所有词都存储
'''
class FindNewTokenOnJieba2:
    def __init__(self, sentences , token_length=4, min_proba={2: 5, 3: 25, 4: 100}):
        '''
            ...
            for char_id in range(len(txt)):
                for step in range(1, self.token_length + 1):
                    if char_id + step <= len(txt):
                        ...
                        ngrams[word] += 1
        self.ngrams = {k: v for k, v in ngrams.items()} # 不设置阈值，结巴分词后所有词都存储
    ...
    def sentences_cut(self):
        ...
        self.all_tokens = {k: v for k, v in all_tokens.items()}
