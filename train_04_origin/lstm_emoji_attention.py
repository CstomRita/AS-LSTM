...
class EMOJI_ATTENTION_LSTM(nn.Module):
    ...
    def forward(self, sentences,all_emojis,device):
        ...
        emoji_ave_embedding = torch.mean(emoji_embeddings,0,True).to(device) # 1 X 1 X 300

        # 2 以sentences分词结果
        indexed = [self.TEXT_VOCAB.stoi[t] for t in sentences]
        sentence_tensor = torch.LongTensor(indexed).unsqueeze(1)
        sentence_embeddings = self.word_embeddings(sentence_tensor).to(device)
        # word_count * 1 * 300
