...
from train_03.lstm_attention import ATTENTION_LSTM
from train_03.word_and_emoji_embedding import Tensor
...
def train(model, data, optimizer, criterion,device):
    ...
    for example in data: # 这里没有使用batch后的iteration，这里的data是整个Example（非向量的一整句话）
        ...
        optimizer.zero_grad()
        predictions = model(sentences=sentence,device=device) # model获取预测结果，此处会执行模型的forWord方法
        if predictions is None:
            print(f'{sentence}|{emoji}|{emotions}')
